...
export const ChatWithJido = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const newMessages: ChatMsg[] = [...messages, { role: 'user', text: userMessage }];
    setMessages(newMessages);
    setInputText('');
    setIsLoading(true);

    try {
      // ✅ Vite standard env var (must start with VITE_)
      ...
      let errorMessage =
        "Beep boop! My connection is a bit fuzzy. Did you know we have special vouchers today? 🎟️";

      if (error?.message === "MISSING_API_KEY") {
        errorMessage =
          "Oops! My API key isn't set up yet. Please add VITE_GEMINI_API_KEY in Vercel Environment Variables.";
      } else if (error?.message === "EMPTY_RESPONSE") {
        errorMessage = "I heard you, but I'm not sure what to say. Could you ask that differently?";
      } else if (String(error?.message || '').includes("403") || String(error?.message || '').includes("API key")) {
      ...
      } else if (String(error?.message || '').includes("503") || String(error?.message || '').includes("500")) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-80 md:w-96 bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[500px] animate-in slide-in-from-bottom-10 fade-in duration-300">
          ...
          <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-slate-100 flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-100 border-0 rounded-full px-4 py-2 text-sm outline-none text-slate-900"
            />
            <button
              type="submit"
              disabled={isLoading}
